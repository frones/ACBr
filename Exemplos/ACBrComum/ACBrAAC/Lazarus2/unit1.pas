...
uses
  ...
  Dialogs, ACBrAAC, ACBrBase, ACBrECF, StdCtrls, ExtCtrls, Buttons,
  ComCtrls, Spin, FileUtil;
...
begin
  ...
  if not FileExistsUTF8(GetPathArquivoAuxiliar) { *Converted from FileExists* } then
  begin
...
begin
  try
    if FileExistsUTF8(GetPathArquivoAuxiliar) { *Converted from FileExists* } then
      DeleteFileUTF8(GetPathArquivoAuxiliar); { *Converted from DeleteFile* }
