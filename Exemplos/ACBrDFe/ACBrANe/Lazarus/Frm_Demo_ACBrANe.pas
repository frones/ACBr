...
uses
  ...
  Dialogs, ComCtrls, StdCtrls, Buttons, ExtCtrls,  pcaConversao, ACBrDFe, ACBrUtil,
  ACBrANe, ACBrMail, ACBrBase, ACBrDFeSSL, Spin, SynMemo, SynEdit;
...
begin
  ...
  try
     ...
     ACBrANe1.Configuracoes.Certificados.NumeroSerie := edtNumSerie.Text;
      ...
      edtPathLogs.Text     := Ini.ReadString( 'Geral','PathSalvar'  ,IncludeTrailingPathDelimiter(ExtractFilePath(Application.ExeName))+'Logs');
      edtPathSchemas.Text  := Ini.ReadString( 'Geral','PathSchemas'  ,IncludeTrailingPathDelimiter(ExtractFilePath(Application.ExeName))+'Schemas\'+
                           GetEnumName(TypeInfo(TVersaoANe), integer(cbVersaoDF.ItemIndex) ));
...
procedure TfrmDemo_ACBrANe.GerarANe;
begin
