...
begin
  ...
  mImp.Lines.Add('</zera>');
  mImp.Lines.Add('Fonte tipo A com Página de Código ' + cbxPagCodigo.Selected.Text);
  ...
  mImp.Lines.Add('');
  mImp.Lines.Add('</FB>Fonte tipo B com Página de Código ' + cbxPagCodigo.Selected.Text);
  mImp.Lines.Add('À noite, vovô Kowalsky vê o ímã cair no pé do pingüim queixoso e vovó põe açúcar no chá de tâmaras do jabuti feliz.');
  mImp.Lines.Add('ÁÉÍÓÚáéíóúçÇãõÃÕÊêÂâÔôÀà');
  mImp.Lines.Add('</corte_total>');
end;
...
begin
  ...
  begin
    mResp := MessageDlg('SIM - Em ASCII Art'+sLineBreak+
                        'NÃO - Em Base64', TMsgDlgType.mtConfirmation, mbYesNoCancel, 0);
