...
type
  ...
  TForm1 = class(TForm)
    ...
    Button1: TButton;
    procedure btnGerarCDClick(Sender: TObject);
...
procedure TForm1.btnGerarCDClick(Sender: TObject);
var
  ip : Integer;
  ine : integer;
begin
  ACBrADRCST1.Layout := lyADRCSTCD;
  ACBrADRCST1.Path := ExtractFilePath(ParamStr(0));
  ACBrADRCST1.Arquivo := 'NomeDoArquivo_CD.txt';

  ACBrADRCST1.Delimitador := '|';
  ACBrADRCST1.CurMascara := '#0.00';
  ACBrADRCST1.IniciaGeracao;

  with ACBrADRCST1.Bloco_0.Registro0001New do
  begin
    COD_VERSAO := vlADRCSTCDVersao100;
    MES_ANO := EncodeDate(2020, 01, 01);
    CNPJ := '00000000000100';
    IE := '0123456789';
    NOME := 'Nome Empresarial até 100';
  ...
  for ip := 1 to 10 do
  begin
    with ACBrADRCST1.Bloco_1.Registro1001New do
    begin
      if (ip mod 2)=0 then
        IND_FECOP := indProdutoNaoSujeitoFECOP
      else
      ...
      with Registro1101 do
      begin
        QTD_TOT_ENTRADA:= Random(200);
        VL_BC_ICMSST_UNIT_MED:= Random(300);
        VL_TOT_ICMS_SUPORT_ENTR:= Random(540);
        VL_UNIT_MED_ICMS_SUPORT_ENTR:= Random(16556);
        QTD_TRANF := Random(16556);


        for ine := 0 to 5 do
          with Registro1111List.New() do
          begin
            DT_DOC:=now;
            COD_RESP_RET:=inRRProprioDeclarante;
            CST_CSOSN:='60';
