...
type
  ...
  protected
    ...
    procedure DefinirAccept; override;
...
procedure TBoletoW_Itau.DefinirAccept;
begin
  FPContentType:= 'application/vnd.itau';
end;
...
begin
  if Assigned(Titulos) then
    FPIdentificador := 'identificador: ' + Titulos.ACBrBoleto.Cedente.CNPJCPF;
end;
...
begin
  ...
  begin
    ...
    try
      Json.Add('tipo_ambiente').Value.AsInteger := 2;
      ...
      Json.Add('subproduto').Value.AsString := '00008';
      ...
      Json.Add('data_vencimento').Value.AsString := FormatDateTime('yyyy-mm-dd', Vencimento);
      Json.Add('valor_cobrado').Value.AsString := IntToStrZero( round( ValorDocumento * 100), 17);
      Json.Add('seu_numero').Value.AsString := SeuNumero;
      Json.Add('especie').Value.AsString := EspecieDoc;
      Json.Add('data_emissao').Value.AsString := FormatDateTime('yyyy-mm-dd', DataDocumento);
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        JsonConta.Add('cpf_cnpj_beneficiario').Value.AsString := OnlyNumber( ACBrBoleto.Cedente.CNPJCPF );
        JsonConta.Add('agencia_beneficiario').Value.AsString  := ACBrBoleto.Cedente.Agencia;
        JsonConta.Add('conta_beneficiario').Value.AsString  := ACBrBoleto.Cedente.Conta;
        JsonConta.Add('digito_verificador_conta_beneficiario').Value.AsString  := ACBrBoleto.Cedente.ContaDigito;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        JsonDadosPagador.Add('logradouro_pagador').Value.AsString := Sacado.Logradouro +' '+Sacado.Numero;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        JsonSacadorAvalista.Add('cpf_cnpj_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.CNPJCPF;
        JsonSacadorAvalista.Add('nome_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.NomeAvalista;
        JsonSacadorAvalista.Add('logradouro_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.Logradouro;
        JsonSacadorAvalista.Add('bairro_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.Bairro;
        JsonSacadorAvalista.Add('cidade_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.Cidade;
        JsonSacadorAvalista.Add('uf_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.UF;
        JsonSacadorAvalista.Add('cep_sacador_avalista').Value.AsString := Sacado.SacadoAvalista.CEP;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        begin
          JsonJuros.Add('data_juros').Value.AsString := FormatDateTime('yyyy-mm-dd', DataMoraJuros);
          if ( (StrToInt64Def(CodigoMora,5)) in [1, 6]) then
            JsonJuros.Add('valor_juros').Value.AsString := IntToStrZero( round( ValorMoraJuros * 100), 17)
          else
            JsonJuros.Add('percentual_juros').Value.AsString := IntToStrZero( round( ValorMoraJuros * 100000), 12);
    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            JsonDesconto.Add('data_desconto').Value.AsString := FormatDateTime('yyyy-mm-dd', DataDesconto);
            if ((Integer(TipoDesconto)) in [1,3,4] ) then
              JsonDesconto.Add('valor_desconto').Value.AsString := IntToStrZero( round( ValorDesconto * 100), 17)
            else
              JsonDesconto.Add('percentual_desconto').Value.AsString := IntToStrZero( round( ValorDesconto * 100000), 12);
          end;
          ...
          begin
            ...
            if DataDesconto > 0 then
            begin
              JsonDesconto.Add('data_desconto').Value.AsString := FormatDateTime('yyyy-mm-dd', DataDesconto2);
              if ((Integer(TipoDesconto2)) in [1,3,4] ) then
                JsonDesconto.Add('valor_desconto').Value.AsString := IntToStrZero( round( ValorDesconto2 * 100), 17)
              else
                JsonDesconto.Add('percentual_desconto').Value.AsString := IntToStrZero( round( ValorDesconto2 * 100000), 12);
            end;
          JsonArrGrupoDesconto.Add.AsObject:= JsonDesconto;
          end;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        JsonRecDivergente.Add('tipo_autorizacao_recebimento').Value.AsString := IntToStr(integer(TipoPagamento));
        ...
        JsonRecDivergente.Add('valor_minimo_recebimento').Value.AsString := IntToStrZero( round( ValorMinPagamento * 100), 17) ;
        JsonRecDivergente.Add('percentual_minimo_recebimento').Value.AsString := IntToStrZero( round( PercentualMinPagamento * 100000), 12);
        JsonRecDivergente.Add('valor_maximo_recebimento').Value.AsString := IntToStrZero( round( ValorMaxPagamento * 100), 17) ;
        JsonRecDivergente.Add('percentual_maximo_recebimento').Value.AsString := IntToStrZero( round( PercentualMaxPagamento * 100000), 12);
