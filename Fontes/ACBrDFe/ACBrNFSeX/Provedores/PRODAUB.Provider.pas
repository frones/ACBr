...
type
  ...
  TACBrNFSeProviderPRODAUB204 = class (TACBrNFSeProviderABRASFv2)
  protected
...
procedure TACBrNFSeProviderPRODAUB204.Configuracao;
begin
...
begin
  Result := TNFSeW_PRODAUB204.Create(Self);
  Result.NFSe := ANFSe;
end;

function TACBrNFSeProviderPRODAUB204.CriarLeitorXml(
  const ANFSe: TNFSe): TNFSeRClass;
begin
  Result := TNFSeR_PRODAUB204.Create(Self);
  Result.NFSe := ANFSe;
end;

function TACBrNFSeProviderPRODAUB204.CriarServiceClient(
...
begin
  ...
  if URL <> '' then
    Result := TACBrNFSeXWebservicePRODAUB204.Create(FAOwner, AMetodo, URL)
  else
  begin
...
function TACBrNFSeXWebservicePRODAUB204.TratarXmlRetornado(
