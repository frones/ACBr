...
unit DebitoAutomatico.Santander.Provider;
...
type
  ...
  TACBrDebitoAutomaticoProviderSantander = class (TFebraban150Provider)
  protected
...
{ TACBrDebitoAutomaticoProviderSantander }

function TACBrDebitoAutomaticoProviderSantander.CriarGeradorTxt(
  const aDebitoAutomatico: TDebitoAutomatico): TArquivoWClass;
begin
  Result := TArquivoW_Santander.Create(Self);
  Result.DebitoAutomatico := aDebitoAutomatico;
end;

function TACBrDebitoAutomaticoProviderSantander.CriarLeitorTxt(
  const aDebitoAutomatico: TDebitoAutomatico): TArquivoRClass;
begin
  Result := TArquivoR_Santander.Create(Self);
  Result.DebitoAutomatico := aDebitoAutomatico;
end;
