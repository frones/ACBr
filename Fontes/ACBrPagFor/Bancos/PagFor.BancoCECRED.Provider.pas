...
unit PagFor.BancoCECRED.Provider;
...
type
  ...
  TACBrPagForProviderBancoCECRED = class (TCNAB240Provider)
  protected
...
{ TACBrPagForProviderBancoCECRED }

function TACBrPagForProviderBancoCECRED.CriarGeradorTxt(
  const aPagFor: TPagFor): TArquivoWClass;
begin
  Result := TArquivoW_BancoCECRED.Create(Self);
  Result.PagFor := aPagFor;
end;

function TACBrPagForProviderBancoCECRED.CriarLeitorTxt(
  const aPagFor: TPagFor): TArquivoRClass;
begin
  Result := TArquivoR_BancoCECRED.Create(Self);
  Result.PagFor := aPagFor;
end;
