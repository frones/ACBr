...
type
  ...
  TACBrPagForProviderBradesco = class (TCNAB240Provider)
  protected
...
{ TACBrPagForProviderBradesco }

function TACBrPagForProviderBradesco.CriarGeradorTxt(
  const aPagFor: TPagFor): TArquivoWClass;
begin
  Result := TArquivoW_Bradesco.Create(Self);
  Result.PagFor := aPagFor;
end;

function TACBrPagForProviderBradesco.CriarLeitorTxt(
  const aPagFor: TPagFor): TArquivoRClass;
begin
  Result := TArquivoR_Bradesco.Create(Self);
  Result.PagFor := aPagFor;
end;
