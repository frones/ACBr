...
const
  ...
  PWOPER_RESTRICOES       = 3; // Restrincoes de transação no Elgin/Destaxa
  PWOPER_TRANSHABILITADA  = 4; // Transacoes Habilitadas no Elgin/Destaxa
  ...
  PWINFO_REDECNPJ       = 29;     // CNPJ da Rede/Credenciadora, sem formatação.
...
type
  EACBrTEFElginIDHWeb = class(EACBrTEFErro);

  { TACBrTEFRespElginIDHWeb }

  TACBrTEFRespElginIDHWeb = class( TACBrTEFResp )
  public
  ...
  begin
    ...
    begin
      ...
      case Linha.Identificacao of
        ...
        begin
          Rede := LinStr;
          NFCeSAT.CNPJCredenciadora := LinStr;
          if (Trim(Rede) <> '') then
          begin
            //Verificar se o Elgin/Destaxa retorna isso ou vamos tratar igual ao paygo
        ...
        PWINFO_FINTYPE:
        begin
          //Elgin retorna igual ao Sitef “00”: À vista; “01”: Pré-Datado; “02”: Parcelado Estabelecimento; “03”: Parcelado Administradora
          AInt := Linha.Informacao.AsInteger;
          if (AInt = 0) then
          begin
...
begin
  ...
  ConteudoToPropertyElginIDHWeb( ATefResp );
end;
...
begin
  case codRetorno of
    ...
    8: Result := 'Nao encontrou a Elgin ou ela esta com problemas';
    9: Result := 'Configuracao de servidores Elgin foi excedida.';
    10: Result := 'Erro de acesso na pasta Elgin (possivel falta de permissao para escrita)';
    ...
    -5: Result := 'Sem comunicacao com o Elgin.';
    ...
    -13: Result := 'Documento fiscal nao encontrado nos registros da Elgin. Retornado em funcoes de consulta tais como ObtemQuantidadeTransacoesPendentes.';
    ...
    -40: Result := 'Transacao negada pelo servidor Elgin.';
...
procedure TACBrTEFRespElginIDHWeb.ConteudoToProperty;
begin
  ConteudoToPropertyElginIDHWeb( Self );
end;
