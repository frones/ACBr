...
/**
 * Implementa AutoCloseable para ser possível usar em try-with-resources Uso:
 * try( ACBrCTe wrapper = new ACBrCTe(...) ){ nfe.statusServico(); } Dessa forma
 * ele retirará o ACBrCTe da memória ao terminar de utilizar, então não precisa
 * do bloco finally, para setar tirar o nfe da memória, pois ele chama o
 * finalize() no método close();
 *
 * É melhor utilizar em um try-with-resources, que depender do método
 * finalize(), pois o finalize() será chamado somente quando for efetuado o
 * garbage collection do objeto, então fica impossível determinar QUANDO e SE o
 * finalize() será chamado. Já o try-with-resources chama o close() ao finalizar
 * seu escopo.
 */
