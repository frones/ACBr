...
try {
    ...
    if (ConfigGravarValor($handle, $ffi, "CEP", "Usuario", (string)$usuario) != 0) exit;
    if (ConfigGravarValor($handle, $ffi, "CEP", "Senha", (string)$senha) != 0) exit;
    if (ConfigGravarValor($handle, $ffi, "CEP", "ChaveAcesso", (string)$chaveacesso) != 0) exit;
} catch (Exception $e) {
    $erro = $e->getMessage();
