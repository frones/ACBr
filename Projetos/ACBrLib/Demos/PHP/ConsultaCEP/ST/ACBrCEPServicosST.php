...
try {
    ...
    if ($metodo == "salvarConfiguracoes") {
        ...
        if (ConfigGravarValor($ffi, "CEP", "Usuario", $_POST['usuario']) != 0) exit;
        if (ConfigGravarValor($ffi, "CEP", "Senha", $_POST['senha']) != 0) exit;
        if (ConfigGravarValor($ffi, "CEP", "ChaveAcesso", $_POST['chaveacesso']) != 0) exit;
        if (ConfigGravarValor($ffi, "CEP", "WebService", $_POST['webservice']) != 0) exit;
    ...
    if ($metodo == "carregarConfiguracoes") {
        ...
        if (ConfigLerValor($ffi, "CEP", "Usuario", $usuario) != 0) exit;
        if (ConfigLerValor($ffi, "CEP", "Senha", $senha) != 0) exit;
        if (ConfigLerValor($ffi, "CEP", "ChaveAcesso", $chaveacesso) != 0) exit;
        if (ConfigLerValor($ffi, "CEP", "WebService", $webservice) != 0) exit;
    ...
    if (($metodo == "BuscarPorCEP") || ($metodo == "BuscarPorLogradouro")) {
        ...
        if ($metodo == "BuscarPorCEP")
            $resultado = BuscarPorCEP($ffi, $_POST['cepcons'], $iniContent);
        else
            $resultado = BuscarPorLogradouro($ffi, $_POST['cidadecons'], $_POST['tipocons'], $_POST['logradourocons'], $_POST['ufcons'], $_POST['bairrocons'], $iniContent);
