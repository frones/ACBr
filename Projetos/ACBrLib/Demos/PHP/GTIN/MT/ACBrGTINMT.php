...
{
    $retorno = $ffi->GTIN_Inicializar(FFI::addr($handle), $iniPath, "");
...
{
    $retorno = $ffi->GTIN_Finalizar($handle->cdata);
...
{
    $retorno = $ffi->GTIN_ConfigLer($handle->cdata, $eArqConfig);
...
function ConfigLerValor($handle, $ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->GTIN_ConfigLerValor($handle->cdata, $eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->GTIN_ConfigGravarValor($handle->cdata, $eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[535]");
...
{
    $retorno = $ffi->GTIN_ConfigGravar($handle->cdata, $eArqConfig);
    $sMensagem = FFI::new("char[535]");
...
function UltimoRetorno($handle, $ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        $esTamanho = FFI::new("long");
        $esTamanho->cdata = 9048;
        $resposta = $ffi->GTIN_UltimoRetorno($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function GTINNome($handle, $ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_Nome($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function GTINVersao($handle, $ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_Versao($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function OpenSSLInfo($handle, $ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_OpenSSLInfo($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function ObterCertificados($handle, $ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_ObterCertificados($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function Consultar($handle, $ffi, $AGTIN, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->GTIN_Consultar($handle->cdata, $AGTIN, $sMensagem, FFI::addr($esTamanho));

    if ($retorno !== 0) {
        if (UltimoRetorno($handle, $ffi, $retorno, $sMensagem, "Erro ao consultar o GTIN", 1) != 0)
            return -10;
    }
