...
{
    $retorno = $ffi->GTIN_Inicializar($iniPath, "");
...
{
    $retorno = $ffi->GTIN_ConfigLer($eArqConfig);
...
function ConfigLerValor($ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->GTIN_ConfigLerValor($eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->GTIN_ConfigGravarValor($eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[535]");
...
{
    $retorno = $ffi->GTIN_ConfigGravar($eArqConfig);
    $sMensagem = FFI::new("char[535]");
...
function UltimoRetorno($ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        $esTamanho = FFI::new("long");
        $esTamanho->cdata = 9048;
        $resposta = $ffi->GTIN_UltimoRetorno($sMensagem, FFI::addr($esTamanho));
...
function GTINNome($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_Nome($sMensagem, FFI::addr($esTamanho));
...
function GTINVersao($ffi, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_Versao($sMensagem, FFI::addr($esTamanho));
...
function OpenSSLInfo($ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_OpenSSLInfo($sMensagem, FFI::addr($esTamanho));
...
function ObterCertificados($ffi, &$retornoGeral)
{
    ...
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[535]");
    $retorno = $ffi->GTIN_ObterCertificados($sMensagem, FFI::addr($esTamanho));
...
function Consultar($ffi, $AGTIN, &$retornoGeral)
{
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $sMensagem = FFI::new("char[9048]");
    $retorno = $ffi->GTIN_Consultar($AGTIN, $sMensagem, FFI::addr($esTamanho));

    if ($retorno !== 0) {
        if (UltimoRetorno($ffi, $retorno, $sMensagem, "Erro ao consultar o GTIN", 1) != 0)
            return -10;
    }
