...
{
    $retorno = $ffi->IBGE_Inicializar(FFI::addr($handle), $iniPath, "");
...
{
    $retorno = $ffi->IBGE_Finalizar($handle->cdata);
...
function ConfigLerValor($handle, $ffi, $eSessao, $eChave, &$sValor)
{
    ...
    $retorno = $ffi->IBGE_ConfigLerValor($handle->cdata, $eSessao, $eChave, $sResposta, FFI::addr($esTamanho));
...
{
    $retorno = $ffi->IBGE_ConfigGravarValor($handle->cdata, $eSessao, $eChave, $value);
    $sMensagem = FFI::new("char[9048]");
...
function UltimoRetorno($handle, $ffi, $retornolib, &$sMensagem, $msgErro, $retMensagem = 0)
{
    if (($retornolib !== 0) || ($retMensagem == 1)) {
        $esTamanho = FFI::new("long");
        $esTamanho->cdata = 9048;
        $ffi->IBGE_UltimoRetorno($handle->cdata, $sMensagem, FFI::addr($esTamanho));
...
function BuscarPorCodigo($handle, $ffi, $ACodMun, &$iniContent)
{
    $sResposta = FFI::new("char[9048]");
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $retorno = $ffi->IBGE_BuscarPorCodigo($handle->cdata, $ACodMun, $sResposta, FFI::addr($esTamanho));

    $sMensagem = FFI::new("char[9048]");

    if ($retorno < 0) {
        if (UltimoRetorno($handle, $ffi, $retorno, $sMensagem, "Erro ao consultar o Cód. Município.", 1) != 0)
            return -10;
    }
...
function BuscarPorNome($handle, $ffi, $cidadecons, $ufcons, &$iniContent)
{
    $sResposta = FFI::new("char[9048]");
    $esTamanho = FFI::new("long");
    $esTamanho->cdata = 9048;
    $retorno = $ffi->IBGE_BuscarPorNome($handle->cdata, $cidadecons, $ufcons, 0, $sResposta, FFI::addr($esTamanho));

    $sMensagem = FFI::new("char[9048]");

    if ($retorno < 0) {
        if (UltimoRetorno($handle, $ffi, $retorno, $sMensagem, "Erro ao consultar por Cidade e UF.", 1) != 0)
            return -10;
    }
