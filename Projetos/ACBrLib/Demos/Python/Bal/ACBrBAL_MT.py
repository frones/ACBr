...
def LerPeso():
    ...
    #Iniciando a LIbBal
    nRetorno = acbr_lib.BAL_Inicializar(byref(ponteiro), PATH_INI.encode("utf-8"), "".encode("utf-8"))
    if nRetorno != 0:
        exit(0)

    # VOCE CONFIGURAR NO INI "Porta" | "Modelo" | "Baud"
    acbr_lib.BAL_Ativar(ponteiro)
    ...
    while True:
        ...
        while True:
            ...
            funcao = acbr_lib.BAL_LePesoStr(ponteiro, MillisecTimeOut, sResposta, ctypes.byref(esTamanho))
            if funcao == 0:
                define_bufferResposta(esTamanho.value)
                acbr_lib.BAL_UltimoPesoLidoStr(ponteiro, sResposta,ctypes.byref(esTamanho))
                if nCONTINUA == 0:
    ...
    acbr_lib.BAL_Desativar(ponteiro)
    acbr_lib.BAL_Finalizar(ponteiro)

def menu():
    print("ACBrBal MulTThread")
    print("-"*20)    
