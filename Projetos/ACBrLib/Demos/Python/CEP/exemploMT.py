...
acbr_lib.CEP_Inicializar.argtypes = POINTER(POINTER(c_int)), c_char_p, c_char_p
acbr_lib.CEP_Inicializar.restype = c_int

# Inicializa Lib CEP
resposta = acbr_lib.CEP_Inicializar(ponteiro,PATH_ACBRLIB.encode("utf-8"),"".encode("utf-8"))
if resposta == 0:
    print('CEP_Inicializar | Sucesso Código: ',resposta)
else:    
    print('CEP_Inicializar | Erro Código: ',resposta)
    sys.exit(1)

#configurando tipo de resposta retorno 
acbr_lib.CEP_ConfigGravarValor(ponteiro,"Principal".encode("utf-8"), "TipoResposta".encode("utf-8"), str(2).encode("utf-8"))

#Configurando o log da Biblioteca
acbr_lib.CEP_ConfigGravarValor(ponteiro,"Principal".encode("utf-8"), "LogNivel".encode("utf-8"), str(4).encode("utf-8"))
acbr_lib.CEP_ConfigGravarValor(ponteiro,"Principal".encode("utf-8"), "LogPath".encode("utf-8"), PATH_LOG.encode("utf-8"))
...
def configuraWS():
    ...
    acbr_lib.CEP_ConfigGravarValor(ponteiro,"CEP".encode("utf-8"), "WebService".encode("utf-8"), LServico.encode("utf-8"))
    acbr_lib.CEP_ConfigGravarValor(ponteiro,"CEP".encode("utf-8"), "Usuario".encode("utf-8"), LUsuario.encode("utf-8"))
    acbr_lib.CEP_ConfigGravarValor(ponteiro,"CEP".encode("utf-8"), "Senha".encode("utf-8"), LSenha.encode("utf-8"))
    acbr_lib.CEP_ConfigGravarValor(ponteiro,"CEP".encode("utf-8"), "ChaveAcesso".encode("utf-8"), LChave.encode("utf-8"))
...
def consultaCEP():
    ...
    if not LCEP:
        ...
        # Verificação de tipo para CEP_BuscarPorCEP
        resultado = acbr_lib.CEP_BuscarPorCEP(ponteiro, LCEP.encode("utf-8"), sResposta, ctypes.byref(esTamanho))
        if resultado == 0:
...
def consultaEncereco():
    ...
    if not LTipo or not LEndereco or not LCidade or not LEstado:
        ...
        define_bufferResposta(4096)
        resultado = acbr_lib.CEP_BuscarPorLogradouro(ponteiro, LCidade.encode("utf-8"), LTipo.encode("utf-8"), LEndereco.encode("utf-8"), LEstado.encode("utf-8"), LBairro.encode("utf-8"), sResposta, ctypes.byref(esTamanho))
...
while LOpcao != 4:
    ...
    print('''
    --------------------------------------------------------------------------
    Como usar ACBrLib para Consultar CEP - MultThread
    -----------------------------Menu Principal-------------------------------
    [1] Configurar
    [2] Consultar CEP
...
# Verificação de tipo para CEP_Finalizar
#acbr_lib.CEP_Finalizar.argtypes = POINTER(POINTER(c_int))
#acbr_lib.CEP_Finalizar.restype = c_int
