...
function PortasSeriais(const Device: TACBrDevice): ansistring;
...
function PortasSeriais(const Device: TACBrDevice): ansistring;
var
  ListPorts: TStringList;
  i: Integer;
begin
  Result := '';
  ListPorts:= TStringList.Create;
  try
    Device.AcharPortasSeriais(ListPorts);
...
begin
  ...
  ListPorts:= TStringList.Create;
  try
    Device.AcharPortasRAW(ListPorts);
    for i:= 0 to ListPorts.Count - 1 do
    begin
      Result:= Result + ListPorts[i] + '|';
    end;
