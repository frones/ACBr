...
function NCM_Finalizar(libHandle: PLibHandle): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
...
function NCM_UltimoRetorno(const libHandle: PLibHandle; const sMensagem: PAnsiChar; var esTamanho: Integer): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
function NCM_ConfigImportar(const libHandle: PLibHandle; const eArqConfig: PAnsiChar): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
function NCM_ConfigExportar(const libHandle: PLibHandle; const sMensagem: PAnsiChar; var esTamanho: Integer): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
...
function NCM_ConfigGravar(const libHandle: PLibHandle; const eArqConfig: PAnsiChar): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
function NCM_ConfigLerValor(const libHandle: PLibHandle; const eSessao, eChave: PAnsiChar; sValor: PAnsiChar;
...
function NCM_Inicializar(var libHandle: PLibHandle; const eArqConfig, eChaveCrypt: PAnsiChar): Integer;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
begin
  Result := LIB_Inicializar(libHandle, TACBrLibNCMs, eArqConfig, eChaveCrypt);
end;
...
begin
  try
    VerificarLibInicializada(libHandle);
    Result := TACBrLibNCMs(libHandle^.Lib).BuscarPorCodigo(cNCM, sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
...
begin
  try
    VerificarLibInicializada(libHandle);
    Result := TACBrLibNCMs(libHandle^.Lib).BuscarPorDescricao(cDesc, nTipo, sResposta, esTamanho);
  except
    on E: EACBrLibException do
      Result := E.Erro;
