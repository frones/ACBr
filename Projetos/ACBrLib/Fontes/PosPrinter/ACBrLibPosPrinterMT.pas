...
function POS_UltimoRetorno(const libHandle: PLibHandle; const sMensagem: PChar; var esTamanho: longint): longint;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
function POS_ImportarConfig(const libHandle: PLibHandle; const eArqConfig: PChar): longint;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
...
function POS_LerCaracteristicas(const libHandle: PLibHandle; const sResposta: PChar; var esTamanho: longint): longint;
  {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
function POS_GetPosPrinter(const libHandle: PLibHandle): Pointer;
    {$IfDef STDCALL} stdcall{$Else} cdecl{$EndIf};
{%endregion}
...
begin
  try
    VerificarLibInicializada(libHandle);
    Result := TACBrLibPosPrinter(libHandle^.Lib).GetPosPrinter;
  except
    on E: Exception do
      Result := nil;
  end;
