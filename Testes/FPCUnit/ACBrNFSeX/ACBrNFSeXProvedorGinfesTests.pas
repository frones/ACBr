unit ACBrNFSeXProvedorGinfesTests;
...
type
  ...
  { ACBrNFSeXProvedorGinfesTest }

  ACBrNFSeXProvedorGinfesTest = class(TTestCase)
  private
...
const
  SArquivoGinfes01  = '..\..\..\..\Recursos\NFSe\Provedores\Ginfes\Ginfes_01-nfse.xml';
  UmMunicipioGinfes = 3549805;

{ ACBrNFSeXProvedorGinfesTest }

procedure ACBrNFSeXProvedorGinfesTest.SetUp;
begin
  inherited SetUp;
  FACBrNFSeX1 := TACBrNFSeX.Create(nil);
  FACBrNFSeX1.Configuracoes.Geral.CodigoMunicipio := UmMunicipioGinfes;
end;

procedure ACBrNFSeXProvedorGinfesTest.TearDown;
begin
  FACBrNFSeX1.Free;
  inherited TearDown;
end;

procedure ACBrNFSeXProvedorGinfesTest.AoCriarComponente_ContadorDeNotas_DeveSerZero;
begin
  CheckEquals(0, FACBrNFSeX1.NotasFiscais.Count, 'Contador de NFeS Não é zero');
end;

procedure ACBrNFSeXProvedorGinfesTest.Proverdor_Eh_Ginfes;
begin
  CheckTrue((FACBrNFSeX1.Configuracoes.Geral.Provedor = proGinfes), 'Provedor não é Ginfes');
end;

procedure ACBrNFSeXProvedorGinfesTest.LoadFromFile_PassandoXMLValidoNaoAssinado_CampoXml_EhIdenticoAoArquivo;
var
...
begin
  ...
  try
    ...
    lStrList.LoadFromFile(SArquivoGinfes01);
    sxml := lStrList.Text;
  finally
    lStrList.Free;
  end;
  ...
  FACBrNFSeX1.NotasFiscais.LoadFromFile(SArquivoGinfes01);
  CheckEquals(sxml, FACBrNFSeX1.NotasFiscais[0].XmlNfse, 'Campo XmlNfse não corresponde ao conteúdo do arquivo');

end;

procedure ACBrNFSeXProvedorGinfesTest.LoadFromFile_PassandoXMLValidoNaoAssinado_CamposCarregadosCorretamente;
begin
  FACBrNFSeX1.NotasFiscais.LoadFromFile(SArquivoGinfes01);
...
initialization
  ...
  _RegisterTest('ACBrNFSeXProvedorISSNETTests', ACBrNFSeXProvedorGinfesTest);
